...
- Single-page, two views toggled by state in [app/src/App.vue](app/src/App.vue): `Dashboard` (mode select) → training session. Mode is `Torneo` | `Cash Game` | `Mano Completa`; scenarios are filtered from the loaded DB by `modo`. `Torneo`/`Cash Game` render `Gimnasio` (single-decision); `Mano Completa` renders `ManoCompleta.vue` (multi-street play + step-by-step review).
