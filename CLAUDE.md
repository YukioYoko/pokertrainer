...
- Single-page, two views toggled by state in [app/src/App.vue](app/src/App.vue): `Dashboard` (mode select) → training session. Mode is `Torneo` | `Cash Game` | `Mano Completa` | `Outs`; scenarios are filtered from the loaded DB by `modo`. `Torneo`/`Cash Game` render `Gimnasio` (single-decision); `Mano Completa` renders `ManoCompleta.vue` (multi-street play + step-by-step review); `Outs` renders `OutsTrainer.vue` (pick-the-probability quiz). The shared `EquityBar.vue` also shows the complement (probability the villain holds the best hand = 1 − hero equity).
...
- Each scenario carries a `tipo` discriminator; `validate.py` branches on it. Four variants exist, all still deriving the correct answer from a verifiable source (Regla de Oro):
  ...
  - `hand_full` (modo `"Mano Completa"`, `[Fold, Call]`) — **full multi-street hand** (flop→turn→river), hero defends BB vs a BTN open. A scripted villain bets each street; the hero makes one Call/Fold decision per street (no raising → stays verifiable). Each street's correct answer = `equity(cards to come) >= pot_odds`: Monte Carlo (`poker_math.equity_vs_combos_mc`) on flop/turn, exact on river; villain street range from `cash_ranges.build_street_bet_range` (Janda ratio per bet size). Distinct schema: a `calles` array of per-street decision nodes (each with its own `board`, `math`, `opcion_correcta_index`, and bilingual `explicacion`), plus top-level bilingual `contexto` and `resumen`. Explanations are **deterministic** (`explain._offline_hand`, no LLM). `validate._check_hand` re-derives each node. In the app, folding on any street ends the hand there and the review covers only the streets played ([app/src/components/ManoCompleta.vue](app/src/components/ManoCompleta.vue)). Added by `hand_generator.generate`.
  - `outs` (modo `"Outs"`, options are 4 percentages) — **outs-probability quiz**: hero + board (flop or turn) shown; player picks the % chance to complete the draw by the river. Not a betting decision. An out = an unseen card that lifts hero's category to Straight+ *using hero's hole cards* (on the turn, `_count_outs` requires the hero hand to beat the board-only hand, excluding board-driven flushes/straights); `prob` = exact hypergeometric of hitting ≥1 out. `opcion_correcta_index` = the offered % nearest `prob`. Schema uses `accion_previa` + a single bilingual `explicacion` (no `desglose`). Added by `outs_generator.generate`; `validate._check_outs` re-derives the nearest option; app is [app/src/components/OutsTrainer.vue](app/src/components/OutsTrainer.vue).
  - Multiway/overcall/full-hand/outs are opt-in via `build.py --n-overcall N` / `--n-multiway N` / `--n-hands N` / `--n-outs N`.
